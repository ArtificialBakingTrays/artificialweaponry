...
function SWEP:Reload()
	...
	local owner = self:GetOwner()
	if owner:Armor() < 5 then return end
...
function SWEP:Think()
	...
	local owner = self:GetOwner()
	if 5 > owner:Armor() then
		self:SetDTFloat( 0, 0 )
		return
	end

	local clip1 = self:Clip1() + 1
	if clip1 > self.Primary.ClipSize then return end

	if SERVER then
		owner:SetArmor( owner:Armor() - 5 )
	end

	self:SetClip1( clip1 )
	self:SetDTFloat( 0, 0 )

	self:EmitSound( "weapons/bugbait/bugbait_squeeze3.wav", 75, math.random(140, 150) + (self:Clip1() * 10), 1, 6 )
